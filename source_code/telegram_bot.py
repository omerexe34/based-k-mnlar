...
def send_welcome(message):
    ...
    bot.reply_to(message, "🚴 Freerider VPS Bot Aktif\n\n"
                          ...
                          "/bakim <saat> - Sistemi bakıma al\n"
                          "/bakim kapa veya /bakim bitir - Bakımı kapat\n"
...
def set_maintenance(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "Kullanım: /bakim <saat> (Örn: /bakim 2)\nKapatmak için: /bakim kapa")
        return
        
    MAINTENANCE_FILE = "/var/www/freerider/maintenance.json"
    
    if args[1].lower() in ["kapa", "bitir"]:
        if os.path.exists(MAINTENANCE_FILE):
            os.remove(MAINTENANCE_FILE)
            bot.reply_to(message, "✅ Bakım modu KAPATILDI. Sistem normal şekilde açıldı.")
            
            # Send OneSignal Push Notification
            import requests
            from extensions import ONESIGNAL_APP_ID, ONESIGNAL_API_KEY
            payload = {
                "app_id": ONESIGNAL_APP_ID,
                "included_segments": ["All"],
                "headings": {"en": "Sistem Bakımı Bitti! 🚀"},
                "contents": {"en": "Freerider tekrar aktif. Hemen uygulamaya gir!"}
            }
            headers = {
            ...
            bot.reply_to(message, "Sistem zaten bakım modunda değil.")
        return
    ...
    try:
        ...
        with open(MAINTENANCE_FILE, 'w') as f:
            json.dump(data, f)
        bot.reply_to(message, f"🛠️ Bakım modu AKTİF EDİLDİ.\nSüre: {hours} saat\nBitiş: {data['until']}")
    except ValueError:
        bot.reply_to(message, "Lütfen geçerli bir sayı girin (Örn: 1.5 veya 2)")
