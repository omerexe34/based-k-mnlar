...
(function() {
    const BUILD_STEPS = [
        ...
        { id: 'cassette', name: 'Ruble (Cassette)' },
        ...
        { id: 'extras', name: 'Ekstralar (Örn: Çamurluk vs)' }
    ];
    ...
    window.updateBuildProgress = function(forceStepIndex = -1) {
        ...
        if(currentBuildStepIndex >= BUILD_STEPS.length) {
            ...
            document.getElementById('ai-build-summary-text').innerText = 'Lütfen parçayı girip analiz ettirin.';
    ...
    window.runAI = async function(type) {
        ...
        try {
            ...
            if (type === 'analysis') {
                ...
                html += `<div class="bg-black/60 border border-zinc-700/50 rounded-xl p-3 shadow-lg backdrop-blur-sm">
                    ${warnHtml}
    ...
    window.onBillingEvent = async function(event, data) {
        if (event === "billing_success") {
            ...
            console.log("Ödeme alındı, Google sunucularında doğrulanıyor...");

            try {
                // DİKKAT: URL sonu clever-responder olarak güncellendi!
                ...
                if (result.success) {
                    ...
                    alert("Ödeme doğrulanamadı: " + result.message);
                }
            } catch (error) {
    ...
    startTour: function() {
        ...
        try {
            ...
            const driverObj = driver({
                ...
                allowClose: false,
                steps: [
                    { popover: { title: 'Hoş Geldin! 🚴', description: 'FreeriderTR\'ye katıldığın için harika hissediyoruz. Uygulamayı en iyi şekilde kullanman için detaylı bir tura çıkalım.' } },
                    { element: '#bnav-0', popover: { title: '🗺️ Harita', description: 'Buradan Türkiye\'deki tüm trail ve rampa noktalarını görebilirsin.', side: 'top', align: 'center' }, onHighlightStarted: () => { switchTab(0); } },
                    { element: '#bnav-1', popover: { title: '💬 Chat', description: 'Diğer sürücülerle anlık sohbet et ve etkinlik planla.', side: 'top', align: 'center' }, onHighlightStarted: () => { switchTab(1); } },
                    { element: '#bnav-9', popover: { title: '🎬 Reels', description: 'Sürüş videolarını izle veya kendi anlarını paylaş.', side: 'top', align: 'center' }, onHighlightStarted: () => { switchTab(9); } },
                    { element: '#bnav-ai', popover: { title: '🤖 AI Asistan', description: 'Yapay zeka asistanı ile bisikletini topla ve teknik analiz yap.', side: 'top', align: 'center' } },
                    { element: '#bnav-7', popover: { title: '👤 Profil & Garaj', description: 'Kendi dijital garajını oluştur ve rozetlerini takip et.', side: 'top', align: 'center' }, onHighlightStarted: () => { switchTab(7); } }
                ],
                onDestroyStarted: () => {
                    driverObj.destroy();
                    this.markCompleted();
