<!-- YENİ ŞİFRE BELİRLEME KUTUCUĞU -->
    <div id="new-password-modal" style="z-index: 999999 !important;" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700 scale-in-anim">
            <h2 class="text-3xl text-white mb-3 teko-font tracking-wide text-center">GÜVENLİK UYARISI</h2>
            <p class="text-sm text-zinc-300 mb-5 text-center leading-relaxed">Şifreniz güvenlik nedeni ile sunucudan silinmiştir. Lütfen kendinize yeni bir şifre belirleyin.</p>
            
            <input type="hidden" id="np-username">
            <input type="password" id="np-password" placeholder="Yeni Şifreniz" class="w-full bg-black/60 border border-zinc-700 rounded-xl px-5 py-4 mb-4 text-white text-sm outline-none focus:border-white transition font-bold text-center">
            
            <button onclick="saveNewPassword()" class="w-full bg-gradient-to-r from-blue-600 to-sky-500 hover:from-blue-500 hover:to-sky-400 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-500/20 active:scale-95 transition-all text-sm mb-3">
...
"""
HTML_CODE = <!-- YENİ ŞİFRE BELİRLEME KUTUCUĞU -->
    <div id="new-password-modal" style="z-index: 999999 !important;" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700 scale-in-anim">
            <h2 class="text-3xl text-white mb-3 teko-font tracking-wide text-center">GÜVENLİK UYARISI</h2>
            <p class="text-sm text-zinc-300 mb-5 text-center leading-relaxed">Şifreniz güvenlik nedeni ile sunucudan silinmiştir. Lütfen kendinize yeni bir şifre belirleyin.</p>
...
<head>
    ...
    <style>
        ...
        .introjs-helperLayer { z-index: 999998 !important; }
                .tab-content { padding-bottom: 90px !important; }
    
        /* --- GLOBAL BOTTOM NAV OVERLAP FIX --- */
        #screen-map, #screen-chat, #screen-market, #screen-rank, #screen-news, #screen-missions, #screen-premium, #screen-profile, #screen-referral, #screen-reels, #screen-competition, #screen-giveaway {
...
(function() {
    ...
    <style>
        ...
        #map { margin-bottom: 90px !important; height: 100% !important; width: 100% !important; z-index: 10; touch-action: none; }
        ...
        * { -webkit-tap-highlight-color: transparent; }
        ...
        #bottom-nav {
            background: linear-gradient(135deg, #38bdf8 0%, #2563eb 100%) !important; /* FORCED UPDATE */
        ...
        #map { margin-bottom: 90px !important;
            background: #0a0a0a !important;
            will-change: transform;
        }
...
<body class="fixed inset-0 w-full overflow-hidden bg-black">
    ...
    <div id="app-loading-screen" class="fixed inset-0  flex flex-col items-center justify-center" style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;background:radial-gradient(ellipse 70% 60% at 50% 40%,rgba(0,136,255,0.3) 0%,#000 70%);transition:opacity 0.5s ease;background-color:#000;">
        <!-- Tek arka halka -->
...
<!-- EKRAN ORTASI HEDEF (CROSSHAIR) - Tıklama Sorunu Çözümü -->
                ...
                <div id="comp-bike-detail-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 " onclick="if(event.target === this) this.classList.add('hidden')">
                ...
                <div id="submit-bike-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
                ...
                <!-- Puanlama Modal -->
                <div id="rate-bike-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
                    <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700  scale-in-anim">
        ...
        <div id="social-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="rewards-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
            <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700 relative text-center scale-in-anim">
        ...
        <div id="onboarding-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4  transition-opacity">
        ...
        <div id="forgot-password-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center z-[10000] px-4 ">
            <div class="glass-panel w-full max-w-md rounded-3xl p-6 border border-zinc-700  scale-in-anim">
                ...
                <div id="fp-step-1" class="slide-up-anim">
                    ...
                    <button onclick="closeForgotPasswordModal(); openAISupportPanel(); setTimeout(() => { document.getElementById('ai-support-input').value = 'Şifremi unuttum, e-posta adresimi bilmiyorum veya kaydetmedim. Doğrulama yapabilir miyiz?'; document.getElementById('ai-support-send-btn').click(); }, 500);" class="w-full bg-transparent border border-sky-500/50 hover:bg-sky-500/10 transition text-sky-400 py-3 rounded-xl font-bold text-xs tracking-wide flex items-center justify-center gap-2 mt-3">
                        <span>🤖</span> E-posta ile Kayıt Olmadım / Bilmiyorum
                    </button>
                </div>
        ...
        <div id="add-bike-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="bike-detail-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center z-[10000] px-4 ">
            ...
            <div id="gw-create-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 overflow-y-auto py-10">
            ...
            <div id="gw-join-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
            ...
            <div id="gw-winner-picker-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        ...
        <div id="bottom-nav" class="fixed bottom-0 left-0 right-0 w-full shrink-0 z-[200]" style="display:none;background:#0c4a6e; border-top:1px solid #0284c7; padding:6px 8px max(14px,env(safe-area-inset-bottom)) 8px;">
            ...
            <div class="flex justify-around items-center">
                <button onclick="switchTab(0)" id="bnav-0" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all duration-300 text-blue-200" style="min-width:48px">
...
<button onclick="switchTab(3)" id="bnav-3" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all duration-300 text-blue-200" style="min-width:48px">
                    <span class="text-[22px] leading-none" style="transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1),filter 0.3s ease">🏆</span>
                    <span class="text-[9px] font-black uppercase tracking-widest mt-0.5" style="transition:all 0.2s">Sıra</span>
                </button>
                <button onclick="switchTab(7)" id="bnav-7" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all duration-300 text-blue-200" style="min-width:48px">
                    <span class="text-[22px] leading-none" style="transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1),filter 0.3s ease">👤</span>
                    <span class="text-[9px] font-black uppercase tracking-widest mt-0.5" style="transition:all 0.2s">Profil</span>
                </button>
                <button onclick="toggleSidebar()" id="bnav-menu" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all duration-300 text-blue-200" style="min-width:48px">
                    <span class="text-[22px] leading-none" style="transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1),filter 0.3s ease">☰</span>
        ...
        <div id="settings-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="other-profile-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="google-onboarding-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 py-10 ">
            <div class="glass-panel w-full max-w-md rounded-3xl p-6 border border-zinc-700  scale-in-anim">
        ...
        <div id="chat-rules-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 py-10 ">
        ...
        <div id="market-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 overflow-y-auto py-10 ">
        ...
        <div id="market-detail-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="category-select-modal" class="hidden fixed inset-0 modal-backdrop flex items-end justify-center  px-0">
        ...
        <div id="add-marker-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
            <div class="glass-panel w-full max-w-md rounded-3xl p-6 border border-zinc-700  scale-in-anim">
        ...
        <div id="add-event-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
            <div class="glass-panel w-full max-w-md rounded-3xl p-6 border border-zinc-700  scale-in-anim">
        ...
        <div id="marker-sheet" class="hidden fixed glass-panel border-t border-zinc-700 rounded-t-3xl  p-5  max-h-[88vh] flex flex-col slide-up-anim overflow-hidden" style="bottom:0;left:0;right:0;width:100%;max-width:100%;margin-left:auto;margin-right:auto;box-sizing:border-box;transform:none;">
        ...
        <div id="event-sheet" class="hidden fixed bottom-0 inset-x-0 glass-panel border-t border-zinc-700 rounded-t-3xl  p-5  overflow-y-auto max-h-[88vh] slide-up-anim">
        ...
        <div id="nearby-routes-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
        ...
        <div id="kvkk-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 py-10 ">
        ...
        <!-- ============================================================ -->
        <div id="admin-panel" class="hidden fixed inset-0  flex items-center justify-center px-3" style="background:rgba(0,0,0,0.85);">
        ...
        <div id="news-admin-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4 ">
            <div class="glass-panel w-full max-w-md rounded-3xl p-6 border border-zinc-700  scale-in-anim">
    ...
    <script>
        ...
        async function handleLogin() {
            ...
            try {
                ...
                document.getElementById('np-username').value = res.username;
    ...
    <div id="add-story-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700  scale-in-anim">
    ...
    <!-- YORUM MODALI -->
    <div id="comment-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
    ...
    <div id="support-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-zinc-700  scale-in-anim">
            <div class="flex items-center justify-between mb-5">
    ...
    <div id="danger-modal" class="hidden fixed inset-0 modal-backdrop flex items-center justify-center  px-4">
        <div class="glass-panel w-full max-w-sm rounded-3xl p-6 border border-sky-900/50  scale-in-anim">
